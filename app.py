...
selected_opt=st.sidebar.radio(label="Choose the DB which you waant to chat",options=radio_opt)

if radio_opt.index(selected_opt)==1:
    db_uri = MYSQL
    mysql_host=st.sidebar.text_input("Provide MySQL Host")
    mysql_user=st.sidebar.text_input(" MySQL User")
    mysql_password=st.sidebar.text_input("Provode MySQL password",type="password")
    mysql_db=st.sidebar.text_input("Provode MySQL Database")
...
if not db_uri:
    st.info("Please enter the databse information and uri")
...
def Configure_db(db_uri,mysql_host=None,mysql_user=None,mysql_password=None,mysql_db=None):
    if db_uri==LOCALDB:
        ...
        if not (mysql_host or mysql_user or mysql_password or mysql_db):
            st.error("Please Provide all MySQL connnection details")
            st.stop()
